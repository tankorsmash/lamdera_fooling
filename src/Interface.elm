module Interface exposing (ButtonConfig, ButtonCustomParams, ButtonLinkParams, ButtonParams(..), ButtonTextParams, ButtonType(..), ColorTheme(..), Device, DeviceClass(..), HoveredTooltip(..), Orientation(..), StandardButton, TooltipBody(..), TooltipConfig, TooltipData, TooltipId, TooltipMsg(..), addButtonAttrs, allowUserSelect, blankChar, borderRoundedLeft, borderRoundedRight, border_bottom, border_right, buildTooltipElementConfig, buildTooltipTextConfig, button, buttonWithTooltip, classifyDevice, clipText, colorFromInt, color_black, color_danger, color_danger_bright, color_danger_dark, color_gem, color_grey, color_light_grey, color_off_black, color_pastel_green_1, color_pastel_green_2, color_pastel_green_3, color_pastel_green_4, color_pastel_green_5, color_pastel_green_6, color_pastel_green_7, color_pastel_red_1, color_pastel_red_2, color_pastel_red_3, color_pastel_red_4, color_pastel_red_5, color_pastel_red_6, color_pastel_red_7, color_primary, color_secondary, color_secondary_bright, color_ultra_light_grey, color_very_light_grey, color_very_very_light_grey, color_very_very_very_light_grey, color_white, common_button_attrs, convertColor, cssRule, dangerButtonConfig, decodeColorTheme, defaultBackgroundColor, defaultFontColor, defaultRounded, defaultSolidColor, defaultTextColor, defineHtmlId, deviceClassToString, encodeColorTheme, exampleFontAwesomeLayeredIcon, fontAwesome, font_blood, font_gem, font_grey, getButtonConfig, getRawColorFromHex, getTooltipOffset, hex_to_color, hoveredTooltipMatchesId, monospace, nbsp, noUserSelect, noopAttr, onEnter, optionalAttr, optionalAttrs, orientationToString, outlineButtonConfig, outlineCustomAttrs, outline_button, outline_button_custom, padding_bottom, pointerEventsAll, pointerEventsNone, primaryButtonConfig, primary_button, primary_button_custom, primary_button_tooltip, primary_color_bright, renderBlood, renderBlood_sized, renderBlood_string, renderGem, renderGemSized, renderGemString, renderGp, renderGpSized, renderGpString, renderIf, rgbaToColor, scaled, scaled_font, scrollbarYEl, secondaryButtonConfig, tooltipElem, wrapButtonWithTooltip)
...
rgbaToColor : { a | red : Float, green : Float, blue : Float } -> Element.Color
rgbaToColor { red, green, blue } =
    rgb red green blue


hex_to_color : String -> Element.Color
hex_to_color hex_str =
    Convert.hexToColor hex_str
        |> Result.map (Color.toRgba >> rgbaToColor)
        |> Result.withDefault (rgb255 255 0 0)
...
color_danger_dark : Color
color_danger_dark =
    convertColor Color.darkRed
...
color_very_very_very_light_grey : Color
color_very_very_very_light_grey =
    rgb 0.9 0.9 0.9
...
allowUserSelect : Element.Attribute msg
allowUserSelect =
    Html.Attributes.style "userSelect" "text" |> Element.htmlAttribute
...
optionalAttr : Bool -> Element.Attribute msg -> Element.Attribute msg
optionalAttr condition attr =
    if condition then
        attr
...
optionalAttrs : Bool -> List (Element.Attribute msg) -> List (Element.Attribute msg)
...
border_right : Int -> Element.Attribute msg
border_right width =
    Border.widthEach { bottom = 0, left = 0, right = width, top = 0 }


borderRoundedLeft : Int -> Element.Attribute msg
borderRoundedLeft bord =
    Border.roundEach { topLeft = bord, bottomLeft = bord, topRight = 0, bottomRight = 0 }


borderRoundedRight : Int -> Element.Attribute msg
borderRoundedRight bord =
    Border.roundEach { topLeft = 0, bottomLeft = 0, topRight = bord, bottomRight = bord }
...
{-| Only render an element if the bool is true
Use showIf if you want it to be invisible but still take space on the page
-}
